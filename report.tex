...
All this type of algorithms are based on an objective function $J$ which \emph{quantifies the quality} of the cluster models and **must be minimized** to obtain optimal cluster.
...
\begin{itemize}
    ...
    1771 customers (65.3\%). These customers fit perfectly into standard personas. Standard marketing campaigns for their specific segment will be highly effective.
    \item \textbf{Bridge Customers (Medium/Low Certainty <80\%):}  942 customers (34.7\%). Roughly one-third of the customer base does not fit neatly into a single box.
\end{itemize}
% FIGURE: Distribution
\begin{figure}[H]
    \centering
...
Unlike K-Means, which would force these 942 "Bridge Customers" into a single segment, 
